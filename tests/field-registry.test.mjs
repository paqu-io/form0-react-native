...
const packageTypesSource = readFileSync(new URL('../src/index.d.ts', import.meta.url), 'utf8');
...
test('form renderer exposes parity-facing snapshot and engine APIs', () => {
  ...
  assert.match(formRendererSource, /engineOptions,/, 'FormRenderer should accept engineOptions');
  ...
  assert.match(formRendererSource, /kind: 'seed'/, 'Snapshot callbacks should emit a seed phase');
...
test('repeatable titles use the shared dataset resolver without magic field heuristics', () => {
  assert.match(formRendererSource, /resolveDatasetRowTitle/);
  assert.match(formRendererSource, /titleDatasets\.byRepeatableKey/);
  assert.doesNotMatch(formRendererSource, /fallbackKeys\s*=\s*\['title', 'name', 'label'\]/);
  assert.doesNotMatch(formRendererSource, /title_field\?\.data_name/);
});
...
test('repeatable list and editor use shared header-driven chrome', () => {
  ...
  assert.match(formRendererSource, />View<\/Text>/, 'Repeatable row action label should be View');
